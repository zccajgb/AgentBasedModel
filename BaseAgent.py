...
class BaseAgent():
    ...
    def _convert_to_spherical(array):
        ...
        theta = np.atan(array[1] / array[0])
        phi = np.acos(array[2] / r)
    ...
    def _metropolis_algorithm_for_binding(binding_energy):
        return np.random.uniform(low=0, high=1) > np.exp(-binding_energy)
    
    def _metropolis_algorithm_for_unbinding(binding_energy):
        return np.random.uniform(low=0, high=1) < np.exp(-binding_energy)
